...
def main():
    ...
    df.loc[:, 'minutes_played'] = df['ms_played'] / 60000
    ...
    df.loc[:, 'weekday'] = df['weekday'].map(day_dict)

    # Define the order of days in a week
    day_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']

    # Set 'weekday' column to categorical data type and specify the order
    df.loc[:, 'weekday'] = pd.Categorical(df['weekday'], categories=day_order, ordered=True)
